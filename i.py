...
💡 You can select the pruning settings using the --pruning flag.
    ...
    sed_option = "''" if platform.system() == 'Darwin' else ''
    ...
    if choice == PruningChoice.DEFAULT:
        ...
        subprocess.run(f"sed -i {sed_option} -E 's/pruning = \"default\"/pruning = \"nothing\"/g' {app_toml}", shell=True)

    elif choice == PruningChoice.EVERYTHING:
        primeNum = random.choice([x for x in range(11, 97) if not [t for t in range(2, x) if not x % t]])
        subprocess.run(f"sed -i {sed_option} -E 's/pruning = \"default\"/pruning = \"custom\"/g' {app_toml}", shell=True)
        subprocess.run(f"sed -i {sed_option} -E 's/pruning-keep-recent = \"0\"/pruning-keep-recent = \"10000\"/g' {app_toml}", shell=True)
        subprocess.run(f"sed -i {sed_option} -E 's/pruning-interval = \"0\"/pruning-interval = \"{primeNum}\"/g' {app_toml}", shell=True)
...
def customize_config(home, network):
    ...
    if network == NetworkChoice.TESTNET:
        ...
        subprocess.run(f"sed -i {sed_option} -E 's/minimum-gas-prices = \"0stake\"/minimum-gas-prices = \"0umark\"/g' {app_toml}", shell=True)
        ...
        subprocess.run(f"sed -i {sed_option} -E 's/persistent_peers = \"\"/persistent_peers = \"{peers}\"/g' {config_toml}", shell=True)
